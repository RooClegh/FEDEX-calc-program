...
# 3. 데이터 로드 (중요: 여기서 모든 전처리를 끝내고 '읽기 전용'으로만 사용)
@st.cache_data
def load_and_clean_data():
    ...
    # 존별 데이터 숫자화 (쉼표 제거 후 정수 변환)
    zone_cols = [c for c in df.columns if '존' in c]
    for col in zone_cols:
        df[col] = df[col].astype(str).str.replace(',', '').str.strip()
        df[col] = pd.to_numeric(df[col], errors='coerce').fillna(0).astype(float) # 계산을 위해 float 유지
