...
st.markdown("""
    ...
    /* 핵심: 버튼 높이를 입력창(유류할증료)과 동일하게 맞춤 */
    div.stButton > button:first-child {
        background-color: #FF6600 !important;
        color: white !important;
        height: 2.8rem; /* 입력창과 비슷한 높이 */
        font-weight: bold;
        border: none;
        width: 100%;
        /* 유류할증료 라벨 높이만큼 상단 여백을 주어 수평을 맞춤 */
        margin-top: 28px; 
        font-size: 1rem;
    }
        ...
        with res_c1:
            with st.container(border=True):
                ...
                if ip_val:
                    ...
                    st.metric("합계", f"{int(total_ip):,.0f}원")
        ...
        with res_c2:
            with st.container(border=True):
                ...
                if ie_val:
                    ...
                    st.metric("합계", f"{int(total_ie):,.0f}원")
                else: st.write("IE 미지원")
