...
def load_and_split_data():
    ...
    # IP 데이터(1~35행)와 IE 데이터(37~67행) 슬라이싱
    ...
    def clean(df):
        ...
        # [해결] 중량 데이터를 소수점 1자리로 반올림하여 검색 정확도 향상
        df['중량_num'] = pd.to_numeric(df['중량(Kg)'], errors='coerce').round(1)
        return df
...
if df_ip is None:
    st.error(f"'{FILE_NAME}' 파일을 찾을 수 없습니다. GitHub에 파일이 있는지 확인해주세요.")
else:
    ...
    if submitted:
        ...
        # [해결] 입력받은 중량도 0.5 단위 올림 후 소수점 1자리로 통일 (16.00 -> 16.0)
        up_weight = round(math.ceil(weight * 2) / 2, 1) 
        
        # 데이터 검색 (정확한 매칭)
        ip_match = df_ip[df_ip['중량_num'] == up_weight]
        ie_match = df_ie[df_ie['중량_num'] == up_weight]
        ...
        with res_col1:
            ...
            if not ip_match.empty:
                ...
                st.warning(f"⚠️ {region} {up_weight}kg IP 데이터 없음")
        ...
        with res_col2:
            ...
            if not ie_match.empty:
                ...
                st.warning(f"⚠️ {region} {up_weight}kg IE 데이터 없음")

    # --- 하단 강조 섹션 (깔끔한 롤백 버전) ---
    st.divider()
    
    st.markdown('<p style="color:#FF6600; font-weight:bold; margin-bottom:0px;">⚠️ 운임 주의사항</p>', unsafe_allow_html=True)
    st.markdown('<p style="color:#FF6600; font-size:0.9rem;">본 계산기는 운임표 기반 예측치이며 실제 청구 금액은 화물의 크기(부피 중량), 통관 수수료 등에 따라 달라질 수 있습니다.</p>', unsafe_allow_html=True)
    
    st.write("") 

    st.write("📅 유류할증료 안내")
    st.write("유류할증료 정보는 주 단위로 변동되므로 정확한 확인이 필요합니다.")
    st.markdown('<a href="https://www.fedex.com/ko-kr/shipping/surcharges.html" target="_blank" style="color:#660099; font-size:0.85rem; font-weight:bold; text-decoration:none;">🔗 [FedEx 유류할증료 상세 확인 (클릭)]</a>', unsafe_allow_html=True)
